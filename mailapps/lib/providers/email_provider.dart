...
class EmailProvider with ChangeNotifier {
  ...
  EmailProvider() {
    _initializeDeviceId();
    // Don't auto-initialize in constructor, let splash screen handle it
  }
  
  /// Public method to initialize the email provider
  /// This should be called from splash screen
  ...
  /// Initialize email on app start - smart auto-generation logic
  Future<void> _initializeEmail() async {
    try {
      // Check if this is first time or no email history exists
      final isFirstTime = await StorageService.isFirstTime();
      final lastEmail = await StorageService.getLastEmail();
      
      if (isFirstTime || lastEmail == null) {
        // First time or no email history - auto generate new email
        await _autoGenerateEmail();
      } else {
        // Use last generated email and force refresh from database
        _setLoading(true);
        _currentEmail = lastEmail;
        _connectSocket();
        
        // Force refresh inbox to get latest emails from database
        try {
          await refreshInbox();
        } catch (e) {
          // If refresh fails, still show cached email but log error
          debugPrint('Failed to refresh inbox on startup: $e');
          _setError('Failed to load latest emails: $e');
        }
      ...
      // If anything fails, generate a new email
      await _autoGenerateEmail();
    }
  }

  /// Auto-generate email (internal method)
  Future<void> _autoGenerateEmail() async {
    _setLoading(true);
    try {
      ...
      // Save to storage
      await StorageService.saveLastEmail(_currentEmail!);
